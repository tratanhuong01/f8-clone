...
const ListItemHomeContent = ({ item, study }) => {
    return (
        <div className='course-list'>
            <div className='course-list-top'>
                <div className='course-list-top-left'>
                    <h2>{item.title}</h2>
                    {item.isNew && <span className='course-list-top-left-new'>
                        MỚI
                    </span>}
                </div>
            ...
            <ScrollContainer className='course-list-main'>
                {item.data.map(dt =>
                    <ItemHomeContent key={dt.id} value={dt} type={item.type} />
                )}
            </ScrollContainer>
        </div>
    )
...
const ItemHomeContent = ({ type, value }) => {
    //
        ...
        switch (type) {
            case 'route':
                return (<div>
                    ...
                    {new Intl.NumberFormat(['ban', 'id']).format(value.view)}
                </div>);
            case 'blog':
                return (
                    <div>
                        <img src={value.user.avatar} style={{ borderRadius: '100rem' }} alt='' width={24} height={24} />
                        <b>{`${value.user.firstName} ${value.user.lastName}`}</b>
                        · {Math.floor(value.read / 60)} phút đọc
                    </div>
                )
            case 'video':
                return (
                    <div style={{ gap: '2.5rem' }}>
                        ...
                        <div>
                            ...
                            {new Intl.NumberFormat(['ban', 'id']).format(value.like)}
                        </div>
                        <div>
                            ...
                            {new Intl.NumberFormat(['ban', 'id']).format(value.comment)}
                        </div>
                    </div>
                )
            default:
    ...
    return (
        <div className="course-list-main-item">
            <div className='course-list-main-item-image' style={{
                ...
                {type === 'video' &&
                    <>
                        ...
                        <ButtonComponent className='course-list-main-item-button'>
                            {`${generateTime(Math.floor(value.time / 60))}:${generateTime(Math.floor(value.time % 60))}`}
                        </ButtonComponent>
                    </>
                }
                <div>
            ...
            <div className='course-list-main-item-info'>
                <Link to='' className='' >{value.title}</Link>
                {footer()}
            </div>
        </div>
    )
